...
class RAGChunkMetadata(BaseModel):
    ...
    section_title: Optional[str] = Field(
        None, description="Заглавието на секцията, ако е намерено"
    )
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Кога е създаден записът"
    )
...
class RAGChunk(BaseModel):
    ...
    id: UUID = Field(
        default_factory=uuid4, description="Уникален идентификатор на парчето"
    )
    content: str = Field(..., description="Самият текст на парчето")
    metadata: RAGChunkMetadata = Field(
        ..., description="Метаданните, свързани с този текст"
    )
    ...
    embedding: Optional[List[float]] = Field(
        None, description="Векторно представяне на текста (1536 dim)"
    )
