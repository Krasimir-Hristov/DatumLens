"""
Document processing module for DatumLens RAG.

This module handles PDF loading, AI-based semantic chunking,
and embedding generation for document chunks.
"""
...
def chunk_documents(documents: List[Document]) -> List[Document]:
    ...
    2. Each sentence is converted to a vector (embedding) using
       OpenAI's model.
    ...
    Example:
    Instead of cutting mid-paragraph at 800 characters, it might
    split like:
    ...
    # "Create a new chunk when the semantic difference is in
    # the top 5% of all differences"
    text_splitter = SemanticChunker(
        embeddings=embeddings,
        # Other options: "standard_deviation", "interquartile"
    ...
    for idx, chunk in enumerate(chunks):
        ...
        chunk.metadata["chunking_method"] = "semantic_ai"
...
def create_embeddings_for_chunks(chunks: List[Document]) -> List[Document]:
    ...
    An embedding is a list of numbers (vector) that represents the
    MEANING of text. Similar texts → similar vectors → we can find
    them with math!

    Example:
    "The contract is valid" → [0.2, -0.5, 0.8, ... 1533 more numbers]
    "The agreement is effective" → [0.19, -0.48, 0.82, ...] (Similar!)
