...
def delete_document_by_filename(filename: str) -> dict:
    ...
    This is for the "Delete & Replace" functionality we discussed.
    ...
    How it works:
    Uses Supabase to delete all rows where metadata->>'source_filename' = filename
    The ->> operator extracts the value from the JSON metadata field
    """
    
    supabase = get_supabase_client()
    
    # Delete all chunks with this filename
    # The .eq() with jsonb field uses the ->> operator to extract the value
    response = supabase.table("document_chunks").delete().eq(
